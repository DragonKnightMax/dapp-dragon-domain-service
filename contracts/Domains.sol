...
contract Domains is ERC721URIStorage {
  ...
  Counters.Counter private _tokenIds; // set our NFT’s unique token number.

  // Top Level Domain
  string public tld;
	
  // We'll be storing our NFT images on chain as SVGs (image made with code)
  ...
  // ERC (NFT Collection's Name, NFT's Symbol)
  constructor(string memory _tld) payable ERC721("Dragon Name Service", "DNS") {
    owner = payable(msg.sender);
    tld = _tld;
    console.log("%s name service deployed", _tld);
  }

  // give us the price of a domain based on length, shorter = more expensive
  // pure function, does not read or modify contract state, just a helper function
  ...
  function register(string calldata name) public payable {
    // check if domain not registered by someone
    ...
    // encodePacked: turn strings to bytes and combine them
  ...
  // calldata (non-persistent & cant be modified, takes least amount of gas)
  ...
  // get all domain names registered
  ...
  // check domain validity (between 3 and 10 characters)
